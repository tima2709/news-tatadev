...
export const getSearchedData = async (query, size = 10) => {
    return await apiRequest(`publications/news/?${query}&page_size=${size}`);
}
...
export const getNewsByAuthor = async (author, slug) => {
    return await apiRequest(`publications/news/?author=${author}&page_size=2&exclude=${slug}`)
}
