...
export const getSearchedData = async (query, size = 10, page = 1) => {
    return await apiRequest(`publications/news/?${query}&page_size=${size}&page=${page}`);
}
...
export const getNewsByAuthor = async (author, slug, randomOrder=false) => {
    return await apiRequest(
        `publications/news/?author=${author}&page_size=2&exclude=${slug}&random_order=${randomOrder}&rubric_type=article`
    )
}
